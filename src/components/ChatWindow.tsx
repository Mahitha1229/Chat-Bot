...
const ChatWindow = () => {
  ...
  // On mobile, start with the history drawer closed so the chat is visible immediately
  useEffect(() => {
  ...
  const chatPanel = (
    <div className="flex min-w-0 h-full flex-1 flex-col">
      <div className="flex flex-wrap items-center justify-between gap-2 px-3 sm:px-5 py-3 sm:py-4 border-b border-border bg-chat-surface">
        <div className="flex items-center gap-2 sm:gap-3 min-w-0">
          <div className="flex h-8 w-8 sm:h-9 sm:w-9 shrink-0 items-center justify-center rounded-xl bg-primary/10 text-primary">
            <Bot className="h-4 w-4 sm:h-5 sm:w-5" />
          </div>
          <div className="min-w-0">
            <h1 className="text-sm font-semibold leading-tight truncate">CloudChat AI</h1>
            <p className="hidden sm:block text-[11px] text-muted-foreground leading-tight">
              Powered by Groq · Firebase
            </p>
          </div>
        </div>
        <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
          <span className="hidden md:block text-xs text-muted-foreground mr-1">
            {user?.email}
          </span>
          ...
          <button
            onClick={logout}
            className="flex items-center gap-1.5 text-xs font-medium bg-secondary px-2 py-1.5 rounded-full hover:bg-secondary/80 transition-all"
            title="Log out"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-xs font-medium bg-secondary px-2 py-1.5 sm:px-3 rounded-full hover:bg-secondary/80 transition-all"
          >
            {isHistoryOpen ? <PanelLeftClose className="h-3.5 w-3.5" /> : <PanelLeft className="h-3.5 w-3.5" />}
            <span className="hidden sm:inline">{isHistoryOpen ? "Close USER HISTORY" : "Open USER HISTORY"}</span>
          </button>
          <button
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-xs font-medium bg-destructive/10 text-destructive px-2 py-1.5 sm:px-3 rounded-full hover:bg-destructive/30 transition-all"
          >
  ...
  return (
    <div className="h-dvh w-full bg-background p-0 sm:p-4">
      <div className="h-full overflow-hidden border-0 sm:rounded-2xl sm:border border-border bg-chat-surface shadow-xl">
        {isMobile ? (
          <>
